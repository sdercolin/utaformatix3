...
enum class Strings(val en: String, val ja: String, val zhCN: String, val ru: String) {
    ...
    ProjectZoomDescription(
        ...
        zhCN = "在不改变实际曲速的前提下同时改变Bpm数值与音符长度。例如，在60 bpm的乐曲中使用2倍的缩放，则乐曲变为120bpm，" +
                "同时所有音符的长度也翻倍。",
        ru = "Изменение частоты ударов в минуту и нот параллельно, чтобы сохранить фактическую скорость пения. Например," +
                "с коэффициентом 2 60 ударов в минуту становятся 120 ударов в минуту, и все ноты становятся в два раза длиннее"
    ),
    ProjectZoomWarning(
        ...
        ru = "Текущие настройки могут быть разрушительными, поскольку для измерения головок необходимо переместить некоторые временные сигнатуры."
    ),
    ProjectZooLabel(
    ...
    ExportNotificationPhonemeResetRequiredVSQ(
        ...
        ru = "Фонемы всех нот были установлены на \"a\". Пожалуйста, сбросьте их, чтобы они звучали правильно." 
    ),
    ...
    ExportNotificationPhonemeResetRequiredV5(
        ...
        ru = "Фонемы всех нот были установлены на \"a\". Пожалуйста, воспользуйтесь \"Job\" -> \"Convert Phonemes to Match Languages\"" +
