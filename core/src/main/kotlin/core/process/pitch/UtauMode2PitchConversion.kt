        ...
        dotPitDataSimplified.map { currNote ->
            if (currNote.pitch.isEmpty()) {
                ...
                UtauMode2NotePitchData(
                    ...
                    currNote.pitch.zipWithNext().map {
                        milliSecFromTick(
...
fun pitchFromUtauMode2Track(
    pitchData: UtauMode2TrackPitchData?,
    notes: List<Note>,
    tempos: List<Tempo>,
): Pitch? {
...
private fun List<Pair<Long, Double>>.fixPointsAtLastNote(
    thisNote: Note,
    lastNote: Note?,
) = if (lastNote == null || lastNote.tickOff != thisNote.tickOn) {
    this
} else {
    val fixed =
        this.map {
    ...
    if (lastPoint != null && lastPoint.first < thisNote.tickOn) {
        fixed + (thisNote.tickOn to 0.0)
    } else {
        fixed
    }
}
...
private fun List<Tempo>.bpmForNote(note: Note): Double = this.last { it.tickPosition <= note.tickOn }.bpm
